            ...
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if i in range(self.height) and j in range(self.width):
                    if (i, j) not in self.safes and (i, j) not in self.mines and (i, j) not in self.moves_made:
                        neighbors.add((i, j))
        neighbor_mines = count - mine_counter
        new_sentence = Sentence(neighbors, neighbor_mines)
        self.knowledge.append(new_sentence)

        for sentence in self.knowledge:
            mine_list = list(sentence.known_mines())
            safe_list = list(sentence.known_safes())
            for cell in mine_list:
                self.mark_mine(cell)
            for cell in safe_list:
                self.mark_safe(cell)
            for sentence in self.knowledge:
